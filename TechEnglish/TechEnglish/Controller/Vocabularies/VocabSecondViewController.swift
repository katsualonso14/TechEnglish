...
class VocabSecondViewController: UITableViewController,AVAudioPlayerDelegate, AVSpeechSynthesizerDelegate {
    ...
    func CustomCellTapButtonCall(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        let contact = sentenceView.secondSentenceArray[indexPathTapped.section].names[indexPathTapped.row]
        print(contact)
        let hasFavorited = contact.hasFavorited
        
        sentenceView.secondSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited = !hasFavorited
    ...
    func CustomCellTapButtonCall2(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        sentenceView.secondSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited2 = !hasFavorited
        //タップしてときの値をpushメッセージに記載
        content.title = contact.name
        content.body = contact.name
        content.sound = UNNotificationSound.default
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return sentenceView.secondSentenceArray[0].names.count
    }
    //cellの中身
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.secondVC = self
        let contact = sentenceView.secondSentenceArray[0].names[indexPath.row]
        //cellの文字指定
        cell.setCell(sentence: contact.name, pronunciation: sentenceView.secondPronunciation[indexPath.row], japanese: sentenceView.secondEnglish[indexPath.row])
...
//    セルの高さ
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        //英語の読み上げ設定
        let utterance = AVSpeechUtterance.init(string: sentenceView.secondSentence[indexPath.row])
